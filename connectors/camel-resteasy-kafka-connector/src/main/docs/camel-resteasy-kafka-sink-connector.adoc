...
| *camel.sink.endpoint.restEasyHttpBindingRef* | Sets the restEasyHttpBinding if you have a customised one registered in the context | null | MEDIUM
...
| *camel.sink.endpoint.proxyMethod* | Sets the proxy method defined in an interface | null | MEDIUM
...
| *camel.sink.endpoint.basicAuth* | Sets the flag to basicAuth on endpoint | null | MEDIUM
...
| *camel.component.resteasy.cookieStore* | To use a custom org.apache.http.client.CookieStore. By default the org.apache.http.impl.client.BasicCookieStore is used which is an in-memory only cookie store. Notice if bridgeEndpoint=true then the cookie store is forced to be a noop cookie store as cookie shouldn't be stored as we are just bridging (eg acting as a proxy). | null | MEDIUM
