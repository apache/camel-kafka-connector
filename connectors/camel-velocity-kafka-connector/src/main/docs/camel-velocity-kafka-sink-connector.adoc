...
The camel-velocity sink connector supports 14 options, which are listed below.
...
| *camel.sink.endpoint.loaderCache* | Enables / disables the velocity resource loader cache which is enabled by default | true | false | MEDIUM
| *camel.sink.endpoint.propertiesFile* | The URI of the properties file which is used for VelocityEngine initialization. | null | false | MEDIUM
...
| *camel.component.velocity.autowiredEnabled* | Whether autowiring is enabled. This is used for automatic autowiring options (the option must be marked as autowired) by looking up in the registry to find if there is a single instance of matching type, which then gets configured on the component. This can be used for automatic configuring JDBC data sources, JMS connection factories, AWS Clients, etc. | true | false | MEDIUM
| *camel.component.velocity.velocityEngine* | To use the VelocityEngine otherwise a new engine is created | null | false | MEDIUM
|===
