...
The camel-string-template sink connector supports 12 options, which are listed below.
...
| *camel.sink.endpoint.delimiterStart* | The variable start delimiter | "<" | false | MEDIUM
| *camel.sink.endpoint.delimiterStop* | The variable end delimiter | ">" | false | MEDIUM
...
| *camel.component.string-template.autowiredEnabled* | Whether autowiring is enabled. This is used for automatic autowiring options (the option must be marked as autowired) by looking up in the registry to find if there is a single instance of matching type, which then gets configured on the component. This can be used for automatic configuring JDBC data sources, JMS connection factories, AWS Clients, etc. | true | false | MEDIUM
|===
