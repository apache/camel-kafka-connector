...
| *camel.source.endpoint.streamIteratorType* | Defines where in the DynamoDB stream to start getting records. Note that using FROM_START can cause a significant delay before the stream has caught up to real-time. One of: [FROM_LATEST] [FROM_START] | "FROM_LATEST" | false | MEDIUM
...
| *camel.component.aws2-ddbstream.streamIteratorType* | Defines where in the DynamoDB stream to start getting records. Note that using FROM_START can cause a significant delay before the stream has caught up to real-time. One of: [FROM_LATEST] [FROM_START] | "FROM_LATEST" | false | MEDIUM
